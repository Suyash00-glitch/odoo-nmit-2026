...
import { Star, ShieldCheck, CheckCircle2, Building, Award, UserCheck } from "lucide-react";
...
export default function SocialProofSection() {
  ...
  return (
    <section id="proof" className="py-24 px-6 bg-black text-white border-t border-neutral-900">
      <div className="max-w-6xl mx-auto space-y-16">
        {/* Header */}
        <div className="text-center max-w-2xl mx-auto space-y-3">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-yellow-500/10 border border-yellow-500/20 text-yellow-400 text-xs font-semibold uppercase tracking-wider">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {testimonials.map((item, idx) => (
            <div
              key={idx}
              className="p-6 rounded-2xl bg-neutral-950 border border-neutral-800 flex flex-col justify-between space-y-6 hover:border-neutral-700 transition-all"
            >
              ...
              <div className="flex items-center gap-3 pt-4 border-t border-neutral-900">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-neutral-400">{item.role} • {item.company}</p>
                </div>
              </div>
            </div>
          ))}
